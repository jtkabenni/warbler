...
def profile(user_id):
    ...
    if form.validate_on_submit():
        ...
        if auth_user:
            ...
            db.session.commit()
            flash(f"Profile updated, {user.username}!", "success")
            return redirect(f"/users/{user_id}")
        flash(f"Password entered was not correct, {user.username}!", "danger")
        return redirect(f"/users/{user_id}")
...
def homepage():
    ...
    if g.user:
        ...
        messages = (Message
                    .query
                    .filter(Message.user_id.in_([user.id for user in g.user.following])).order_by(Message.timestamp.desc())
                    .limit(100)
                    .all())
